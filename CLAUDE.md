...
**3 段目を広く取りすぎない。** 推奨案まで書けている設計の選択は「1. そのまま進める」か
「2. キューに積む」であって、報告のたびにユーザーへ投げるものではない。
自分たちの管理下にあるリポジトリへの PR も、作業の完了条件に含まれるなら止まらない。

### 報告する事実は確認してから書く

**確認できることを推測で断定しない。** 手戻りは、間違った作業そのものより
「間違った前提を渡されたユーザーが動いた分」で大きくなる。とくに次の 3 つ。
...
| `/wrap-up` | セッション終了処理（引き継ぎ・フィードバックの反映・コミット） |
