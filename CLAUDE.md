...
> 目安は「**第三者が 1 分で読み、対応要否を判断できる**」こと。

- **冒頭 2〜3 行で要約する**: 何が起きたか / 何を提案するか / 読み手に何をしてほしいか。本文を読まなくても要約だけで判断できる状態にする
- **本文はスクロール 1 画面（目安 1,000 字）以内**。それを超える調査ログ・検証結果・長いコード引用は `<details>` に畳む
- **1 Issue 1 論点**。提案が複数混ざるなら Issue を分ける
- **見出しは 2 階層まで**。表や箇条書きの入れ子を深くしない
- **代替案は本命 + 最大 2 件**。検討過程の全網羅はせず、求められたらコメントで補足する
- 他 Issue との関係は 1 行のリンクに留める。冒頭の引用ブロックで長い経緯説明をしない
- 投稿前に本文を読み返し、「消しても結論が変わらない段落」を削る
