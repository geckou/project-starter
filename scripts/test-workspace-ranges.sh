...
#   4. 0 系の ^ は minor を固定する（^0.2.0 は 0.3.0 を満たさない。^0.0.3 は patch も固定）
#   5. ~ は minor を固定する
#   6. "*" と workspace: は常に通す
...
# semver では ^0.0.3 は 0.0.3 だけ（patch も固定）
make_repo 0.0.4 '^0.0.3'
output=$(run)
status=$?

if [ "$status" -ne 0 ]; then
  pass "^0.0.3 は 0.0.4 を満たさない"
else
  fail "^0.0.3 は 0.0.4 を満たさない" "$output"
fi

make_repo 0.0.3 '^0.0.3'
output=$(run)

if [ $? -eq 0 ]; then
  pass "^0.0.3 は 0.0.3 を満たす"
else
  fail "^0.0.3 は 0.0.3 を満たす" "$output"
fi
