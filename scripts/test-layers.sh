...
# 参照方式の派生プロジェクトでは必ず落ちる。`.templatesyncignore` の
# template-only の範囲に入れて配らず、ci.yml もリポジトリで判定する（#326）。
# 派生側で層マニフェストと実態が合っているかは check-layers.mjs が見る。
#
# 検証するのは「全部入りのテンプレートからの減算」なので、各層が揃っていることを
# 前提にする。先頭のスキップは、現在の呼び出し元（ci.yml / smoke-test.yml。どちらも
# 本体のフルツリー）からは到達しない。減算後のツリーに対して手元で直接叩いたときの保険。
#
