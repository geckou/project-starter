...
#
# **通常のリリースでこのスクリプトを使う必要はない。** version を上げる PR を production へ
# マージした時点で、publish.yml が未公開のバージョンを自動的に公開する。これを使うのは、
# リリースの区切りを git のタグとして残したいときと、自動公開が破壊的変更の検査に
# 引っかかったので --force で通したいとき。公開されるのはどちらの経路でも
# 「npm に未公開の version」だけなので、公開の後からタグを打っても二重には公開されない。
#
