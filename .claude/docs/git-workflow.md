...
Claude Code の Web / GitHub Action 等のハーネスがセッション用に作るブランチで、名前も分岐元も
こちらでは決められないため、`pre-git-guard.sh` は命名と分岐元の検査を免除している。免除は
**既に `claude/*` にいるセッションの中だけ**で、分岐元も `production` かそのセッションブランチに
限る（`claude/` を付けて検査を外す抜け道にしないため）。
