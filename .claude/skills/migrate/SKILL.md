---
name: migrate
description: 既存リポジトリをこのテンプレートに移植し、.claude/docs/ にドキュメントを自動生成する
---

# migrate

既存プロジェクトのコードを読み取り、このテンプレートの構成に移植する。
企画書・仕様書がない場合は、コードから逆生成して `.claude/docs/` に作成する。
...
2. **プロジェクト名**（例: `my-app`）
3. **移植対象**（Web のみ / Mobile のみ / 両方 / Functions のみ）
...
解析結果をもとに、`.claude/docs/` にドキュメントを生成する。
テンプレートファイル（`planning.md`, `spec.md`, `roadmap.md`）を埋める形で記述する。

#### 企画書 `.claude/docs/planning.md`（コードから推測できる範囲で埋める）
...
#### 仕様書 `.claude/docs/spec.md`（コードからほぼ完全に生成可能）
...
#### ロードマップ `.claude/docs/roadmap.md`

既存の機能は全て「完了」ステータスで記録する。
フェーズは実装内容から自動判定する。
...
### 生成したドキュメント
- 企画書: `.claude/docs/planning.md`
- 仕様書: `.claude/docs/spec.md`
- ロードマップ: `.claude/docs/roadmap.md`
