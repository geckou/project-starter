...
3. 第0層（進め方のスキル。スタック非依存）とスタック層（生成物がスタックに直結する）の
   どちらに属するかを決め、CLAUDE.md「スキル（スラッシュコマンド）」の分類に沿わせる
   （一覧そのものはセッション開始時にハーネスが提示するので、表への追記は不要）
...
- 作成後、CLAUDE.md「スキル（スラッシュコマンド）」の第0層 / スタック層の区別に照らして、
  どちらに属するかを意識して書く
