...
1. ユーザーにスキルの目的を確認する:
   ...
   - どんな入力が必要か
   - どんな出力が期待されるか
2. `.claude/skills/<skill-name>/SKILL.md` を作成する（ディレクトリ + SKILL.md の形式。フラットな `.claude/skills/<skill-name>.md` は Claude Code に認識されない）
...
- スキルは `.claude/skills/<skill-name>/SKILL.md` に1ディレクトリ1スキルで作成する
